...
describe('MuPort', () => {
  ...
  it('create an identity correctly', async () => {
    id1 = await MuPort.createIdentity('lala')
  ...
  it('recover identity correctly', async () => {
    id2 = await MuPort.createIdentity('id2')
    id3 = await MuPort.createIdentity('id3')
    // get dids from three identities and pass them along the identity creation process
    id4 = await MuPort.createIdentity('id4', [id1.getDid(), id2.getDid(), id3.getDid()])
